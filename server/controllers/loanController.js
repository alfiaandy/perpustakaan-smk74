...
// 2. Pengajuan Pinjam oleh Siswa (Via Web OPAC)
exports.requestLoan = async (req, res) => {
  ...
  try {
    ...
    // STEP B: Cari atau Buat Otomatis record di tabel members (penghubung via identity_number)
    let memberId = null;
    ...
    // STEP D: Cek Limit Pinjaman (Maksimal 3 buku)
    const [activeLoans] = await connection.query(
      "SELECT COUNT(*) AS total FROM loans WHERE member_id = ? AND status = 'borrowed'",
      [memberId],
    );
    ...
    // STEP F: Insert Transaksi ke Tabel loans
    ...
    await connection.query(
      `INSERT INTO loans 
        (loan_code, member_id, book_id, loan_date, due_date, status, fine_amount, fine_status, notes) 
       VALUES (?, ?, ?, ?, ?, 'borrowed', 0.00, 'none', 'Pengajuan Online Siswa')`,
      [loanCode, memberId, book_id, loanDate, dueDate],
    );
    ...
    res.status(201).json({
      success: true,
      message:
        "Pengajuan pinjam berhasil! Silakan ambil buku fisik di perpustakaan.",
    });
  } catch (error) {
    await connection.rollback();
    res.status(500).json({ success: false, message: error.message });
